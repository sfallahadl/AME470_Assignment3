...
function loginUser(req, res, db){
  var info = req.query;
  auth.authenticate(req.query.userID, req.query.password, db.collection('users'), db, function(err, user){
    if (user) {
      // if(new Date().getTime() - req.session.lastLogin > 24 * 60 * 60 * 1000){
          //req.session.regenerate(function(){
    
              req.query.userID = "test"; //user.userID;
              req.query.password = "hello"; //user.password;
